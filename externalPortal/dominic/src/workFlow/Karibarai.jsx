...
// ── カラム定義 ──────────────────────────────────────────────
// TODO: kintone 仮払申請アプリのフィールドコードに合わせて調整
const COL_DEFS = [
...
const Karibarai = () => {
    ...
    useEffect(() => {
        const fetchRecords = async () => {
            setLoading(true);
            try {
                const data = await apiPost('workflow/getKaribarai', {});
                setRecords(data || []);
            } catch (err) {
                console.error('仮払申請一覧取得エラー:', err);
                setError('仮払申請一覧の取得に失敗しました。');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <Container component="main" maxWidth="xl">
            <CssBaseline />
            <Box sx={{ mt: 4, mb: 8 }}>
                <PageBreadcrumbs items={[{ label: '仮払申請' }]} />
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    <PageTitle icon={AccountBalanceWalletIcon} sx={{ mb: 0 }}>仮払申請</PageTitle>
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/karibarai/new')}>
                        新規申請
                    </Button>
                </Box>
                <Divider sx={{ mb: 3 }} />
